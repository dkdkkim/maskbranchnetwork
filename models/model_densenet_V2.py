...
class convmodel():
    ...
    def convcam(self):
        ...
        return image, cls_label, is_training, tf.nn.softmax(conv, dim=-1), cam

    def convval_branch(self,th):

        image = tf.placeholder("float32", [None, 48, 32, 48, 1])
        det_label = tf.placeholder("int32", [None, 12, 8, 12])
        expand_dims = tf.placeholder(tf.bool, shape=())
        is_training = tf.placeholder(tf.bool, shape=())
