...
class convmodel():
    ...
    def convnet_weighted(self, zSize, ySize, xSize, alpha=0.0):
        ...
        det_label = tf.placeholder("int32", [None, zSize/4,ySize/4,xSize/4])
        ...
        det_conv_large = tf.nn.avg_pool3d(det_conv_large, (1, 2, 2, 2, 1), strides=(1, 2, 2, 2, 1), padding='SAME')
        ...
        det_conv = det_conv_large + det_conv_small
    ...
    def convval(self, th):
        ...
        for i in range(0, len(self.w2), 3):
            if i == 0:
                conv = self.residual_block_3layers_downsampling(conv, self.w2_init, self.w2[i], self.w2[i + 1],
                                                                self.w2[i + 2], 'conv2_' + str(i), is_training)
            else:
                conv = self.residual_block_3layers(conv, self.w2[i], self.w2[i + 1], self.w2[i + 2], 'conv2_' + str(i),
                                                   is_training)
        det_conv_large = tf.nn.relu(conv)
        det_conv_large = tf.nn.conv3d(det_conv_large, self.fc1_det_large, strides=(1, 1, 1, 1, 1),
                                      padding='SAME') + self.fc1b_det_large
        det_conv_large = tf.nn.avg_pool3d(det_conv_large, (1, 2, 2, 2, 1), strides=(1, 2, 2, 2, 1), padding='SAME')

        for i in range(0, len(self.w3), 3):
            if i == 0:
                conv = self.residual_block_3layers_downsampling(conv, self.w3_init, self.w3[i], self.w3[i + 1],
                                                                self.w3[i + 2], 'conv3_' + str(i),
                                                                is_training)
            else:
                conv = self.residual_block_3layers(conv, self.w3[i], self.w3[i + 1], self.w3[i + 2], 'conv3_' + str(i),
                                                   is_training)
        ###########    V2   ##########
        det_conv_small = tf.nn.relu(conv)
        det_conv_small = tf.nn.conv3d(det_conv_small, self.fc1_det_small, strides=(1, 1, 1, 1, 1),
                                      padding='SAME') + self.fc1b_det_small
        # conv = tf.nn.avg_pool3d(conv, (1, 2, 2, 2, 1), strides=(1, 2, 2, 2, 1), padding='SAME')
        ##############################
        # conv = self._add_transition(conv, self.w4_1x1, 'conv4_1x1', is_training)
        for i in range(0, len(self.w4), 3):
            if i == 0:
                conv = self.residual_block_3layers_downsampling(conv, self.w4_init, self.w4[i], self.w4[i + 1],
                                                                self.w4[i + 2], 'conv4_' + str(i), is_training)
            else:
                conv = self.residual_block_3layers(conv, self.w4[i], self.w4[i + 1], self.w4[i + 2], 'conv4_' + str(i),
                                                   is_training)
        ...
        for i in range(0, len(self.w5), 3):
            if i == 0:
                conv = self.residual_block_3layers_downsampling(conv, self.w5_init, self.w5[i], self.w5[i + 1],
                                                                self.w5[i + 2], 'conv5_' + str(i), is_training)
            else:
                conv = self.residual_block_3layers(conv, self.w5[i], self.w5[i + 1], self.w5[i + 2], 'conv5_' + str(i),
                                                   is_training)
