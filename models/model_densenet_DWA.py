...
class convmodel():
    ...
    def convnet_weighted(self, zSize, ySize, xSize, alpha=0.0):
        ...
        # onehot_labels_reverse = (-1)*(onehot_labels - 1)
        # argmax_onehot_conv_cls = tf.one_hot(tf.cast(tf.argmax(conv,axis=1),tf.int32), depth=2, on_value=1.,off_value=0.)
        # false_positives = tf.reduce_max(tf.multiply(onehot_labels_reverse,argmax_onehot_conv_cls),axis=-1)
        # false_positive_loss = tf.multiply(cross_entropy,false_positives)*false_positive_weight
        # cls_loss += tf.reduce_mean(false_positive_loss)

        ##### trainable weights
        cur_weights = tf.nn.softmax(wk)
        loss = cur_weights[0]*det_loss + cur_weights[1]*cls_loss
        ...
        return image,det_label,cls_label,det_loss,cls_loss,loss,det_acc,cls_acc,\
               is_training, expand_dims, tf.nn.softmax(det_conv,dim=-1),tf.nn.softmax(conv, dim=-1), wk, cur_weights
    ...
    def convtest_class(self, zSize, ySize, xSize):
        ...
        return image, cls_label, cls_loss, cls_acc, is_training, tf.nn.softmax(conv, dim=-1)

    def convtest(self, zSize, ySize, xSize):
        ...
        return image, is_training, tf.nn.softmax(det_conv, dim=-1), tf.nn.softmax(conv, dim=-1)

    def convCAM(self, zSize, ySize, xSize):

        image = tf.placeholder("float32", [self.batch, zSize, ySize, xSize, 1])
        is_training = tf.placeholder(tf.bool, shape=())
        ...
        conv = self.add_transition_wo_pool(conv, self.w4_1x1, 'conv4_1x1', is_training)
        conv = tf.nn.relu(conv)
        conv = tf.nn.conv3d(conv, self.fc1, strides=(1, 1, 1, 1, 1), padding='SAME') + self.fc1b

        return image, is_training, conv, cam_out, tf.squeeze(tf.nn.softmax(conv, dim=-1))
